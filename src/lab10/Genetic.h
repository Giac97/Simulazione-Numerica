...
//Determines wether to perform or not exchange of path in MPI run
extern bool COMMUNICATE;
...
public:
    ...
    void evolve(const std::vector<City>& cities, int generation);
    std::vector<int> tournamentSelection(const std::vector<City>& cities);
    void setBest(std::vector<int> newBest);
    void writeBestLengthMPI(int rank, const std::vector<City>& cities);
    void writeBestPathMPI(int rank, const std::vector<City> &cities, int generation);
private:
    int populationSize_;
    int numCities_;
