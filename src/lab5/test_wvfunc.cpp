...
double psi2_3d(rowvec pt)
{
    ...
    double angular = sqrt(5. / 4.) * (3 * pt(2) * pt(2) - r2) / r2 * sqrt(1. / (4. * M_PI));
    double radial = 1. / (9. * sqrt(30.)) * rho * rho * exp(-rho / 2.);

    double psi = angular * radial;
    return psi * psi;
}
...
int main (int argc, char *argv[]){
   ...
   for (int i = 0; i < M - 1 + relax; i++)
   {
       ...
       double psi2Old = psi2_3d(points.row(i));
       double psi2New = psi2_3d(trial);
   ...
   points.save("psi3d.dat", raw_ascii);
