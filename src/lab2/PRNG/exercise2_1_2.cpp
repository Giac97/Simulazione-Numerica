...
bool sample_first(double x, double r)
{
    double p_x = -2 * (x - 1);
    double p_max = 2.;

    double frac = p_x / p_max;
    bool acc = false;
    if (r < frac)
        acc = true;
    return acc;
...
int main (int argc, char *argv[]){
   ...
   for (int i = 0; i<M; i++)
   {
       ...
       while(!acc)
       {
        x = rnd.Rannyu();
        double r = rnd.Rannyu();
        acc = sample_first(x, r);
       }
       std::cout << x << std::endl;
       r(i) = integrand(x) / (-2. * (x - 1));
   }
    
   r.save("sampled.dat", raw_ascii);
   ...
   out.col(2) = errcol;
	...
	out.save("I_mc_nonuniform.dat", raw_ascii);
