...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < N_walkers - 1; i++)
    {
        ...
        for (int j = 0; j < M - 1; j++)
        {
            ...
            float theta = rnd.Rannyu(0, M_PI);
            float phi = rnd.Rannyu(0, 2.0 * M_PI);
            // Row vector containg the displacement vector to move from step j to j+1
            rowvec mv(3, fill::zeros);
            mv = {sin(theta) * cos(phi), sin(theta) * sin(phi), cos(theta)};
            r.row(j + 1) = r.row(j) + mv;
            d2(j + 1) = distance2(r.row(j + 1));
        }
        if (i == 2)
        {
            r.save("rw_continue_points.dat", raw_ascii);   
        }
    ...
    out_rw.save("rw_continue_distance.dat", raw_ascii);
